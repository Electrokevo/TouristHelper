...
int main() {
  // primero verificacion de ejecucion en gpu
  ...
  if (torch::cuda::is_available()) {
    ...
    std::cout << "CUDA disponible. Usando GPU: " << prop.name << "\n";
  } else {
    std::cout << "CUDA NO disponible. Usando CPU.\n";
  }
  ...
  rf.Train(trainData, trainLabels, encServ.numClasses(), 100, 20);
